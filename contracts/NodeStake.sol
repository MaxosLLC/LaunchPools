...
contract NodeStake is Ownable {
    ...
    modifier validNodeType(uint8 newNodeType) {
        require(newNodeType <= NODETYPE_MAX, "invalid node type");
        _;
    }
    struct NodeInfo {
        uint8 nodeType;
    ...
    IERC20 public stakeToken; // LAUNCHPOOLS token with decimals 18
    ...
    function unstakeInfo(address _user) public view returns (uint256 amount, bool allowed, bool rejected) {
        ...
        UnstakeInfo storage newUnstake = _unstakeInfo[_user];
        if (newUnstake.requestUnstakeTime > currCycleStart) {
            amount = newUnstake.requestUnstakeAmount;
            allowed = newUnstake.rejectUnstakeTime == currCycleStart;
            rejected = newUnstake.rejectUnstakeTime > currCycleStart;
        }
    }
    ...
    function requestUnstake(uint256 _amount) public {
        ...
        UnstakeInfo storage newUnstake = _unstakeInfo[msg.sender];
        newUnstake.requestUnstakeAmount = _amount;
        newUnstake.requestUnstakeTime = block.timestamp;
    ...
    function rejectUnstake(address _user) public onlyManager {
        updateStakeCycle();
        UnstakeInfo storage newUnstake = _unstakeInfo[_user];
        if (newUnstake.requestUnstakeTime > stakeCycleStartTime) {
            newUnstake.rejectUnstakeTime = block.timestamp;
    ...
    function allowUnstake(address _user) public onlyManager {
        ...
        UnstakeInfo storage newUnstake = _unstakeInfo[_user];
        if (newUnstake.requestUnstakeTime > stakeCycleStartTime) {
            newUnstake.rejectUnstakeTime = stakeCycleStartTime;
