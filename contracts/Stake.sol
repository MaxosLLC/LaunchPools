...
abstract contract Stake is Ownable, IERC20 {
    //initialize safe computations
    using SafeMath for uint;
    
    //LaunchPools' staking contract address
    address public launchPools;
    //total amount of LAUNCHPOOLS token staked
    uint public totalStaked;
    //tax rate for staking in percentage
    uint public stakingTaxRate;                     // 10 = 1%
    //tax amount for registration
    uint public registrationTax;
    //daily return on investment (in percentage)
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(launchPools).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough LAUNCHPOOLS to pay registration fee.");
        //makes sure smart contract transfers LAUNCHPOOLS from user
        require(IERC20(launchPools).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked LAUNCHPOOLS amount in the pool
    ...
    function stake(uint _amount) external onlyRegistered() whenActive() {
        ...
        //adds current earnings to stakeRewards
     ...
     * - unregisters stakeholder if all the stakes are removed
     * - emits an {OnStake} event
     */
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(launchPools).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(launchPools).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient LAUNCHPOOLS balance in pool');
        ...
        IERC20(launchPools).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(launchPools).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(launchPools).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient LAUNCHPOOLS balance in pool');
        //transfers _amount to _address
        IERC20(launchPools).transfer(msg.sender, _amount);
        //emit event
